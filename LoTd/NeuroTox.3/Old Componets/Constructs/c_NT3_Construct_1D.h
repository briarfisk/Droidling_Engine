...
public:
     ...
     //Whether or not to flatten the outputs onto a single row when discharging.
     ...
     void discharge_Treetops_C()
     {
          ...
          while(tmp_LL != NULL)
          {
               ...
               if (tmp_LL->NID == NULL)
               {                    
                    if (setting_Flat_Output == 0)
                    {
                         //Creates a new row for the output.
                         tbl_Pattern_Output_C.new_Cell(Current_Input);
                    }
                    else
                    {
                         //Creates a new cell on the same row.
                         tbl_Pattern_Output_C.new_Cell(0);
                    }
               ...
               Nodes.bp(&tbl_Pattern_Output_C, setting_Flat_Output, Current_Input, tmp_LL->NID);
     ...
     void discharge_Treetops_L(int p_Input)
     {
          ...
          while(tmp_LL != NULL)
          {
               ...
               if (tmp_LL->NID == NULL)
               {
                    ...
                    {
                         //Creates a new cell on the same row.
                         tbl_Pattern_Output_L.new_Cell(0);
                    }
     ...
     void discharge_Treetop_By_NID_Into_Given_Tables_L(int p_Position, u_Data_3 p_NID, u_Data_3 p_Charge, int p_Current_Input, c_Raw_Table_1D * p_PTbl, c_Raw_Table_1D * p_PosTbl, c_Raw_Table_1D * p_CTbl, c_Raw_Table_1D * p_RCTbl, c_Raw_Table_1D * p_TtTbl)
     {
          ...
          tmp_Node = Nodes.bp_Treetop(p_PTbl, setting_Flat_Output, p_Current_Input, p_NID);
